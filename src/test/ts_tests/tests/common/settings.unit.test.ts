  ...
  suite('getWorkspaceSettings tests', () => {
    ...
    setup(() => {
      ...
      getConfigurationStub.callsFake((namespace: string, uri: Uri) => {
        if (namespace.startsWith('gaplint')) {
    ...
    test('Default Settings test', async () => {
      ...
      pythonConfigMock
        .setup((c) => c.get('linting.gaplintArgs', []))
        .returns(() => [])
        .verifiable(TypeMoq.Times.never());
      pythonConfigMock
        .setup((c) => c.get('linting.gaplintPath', ''))
        .returns(() => 'gaplint')
        .verifiable(TypeMoq.Times.never());
      pythonConfigMock
      ...
      const settings: ISettings = await getWorkspaceSettings('gaplint', workspace1);
    ...
    test('Resolver test', async () => {
      ...
      configMock
        ...
        .returns(() => [
          '${userHome}/bin/gaplint',
          '${workspaceFolder}/bin/gaplint',
          '${workspaceFolder:workspace1}/bin/gaplint',
          '${cwd}/bin/gaplint',
          '${interpreter}',
        ])
        .verifiable(TypeMoq.Times.atLeastOnce());
      ...
      const settings: ISettings = await getWorkspaceSettings('gaplint', workspace1, true);
      ...
      assert.deepStrictEqual(settings.path, [
        `${process.env.HOME || process.env.USERPROFILE}/bin/gaplint`,
        `${workspace1.uri.fsPath}/bin/gaplint`,
        `${workspace1.uri.fsPath}/bin/gaplint`,
        `${process.cwd()}/bin/gaplint`,
    ...
    test('Legacy Settings test', async () => {
      ...
      pythonConfigMock
        .setup((c) => c.get<string[]>('linting.gaplintArgs', []))
        .returns(() => ['${userHome}', '${workspaceFolder}', '${workspaceFolder:workspace1}', '${cwd}'])
        .verifiable(TypeMoq.Times.never());
      pythonConfigMock
        .setup((c) => c.get('linting.gaplintPath', ''))
        .returns(() => '${userHome}/bin/gaplint')
        .verifiable(TypeMoq.Times.never());
      pythonConfigMock
        .setup((c) => c.get<string[]>('analysis.extraPaths', []))
        .returns(() => [
    ...
    [true, false].forEach((value) => {
      test(`Lint on change settings: ${value}`, async () => {
        configMock
          ...
          .returns(() => value)
          .verifiable(TypeMoq.Times.atLeastOnce());
        assert.deepStrictEqual(isLintOnChangeEnabled('gaplint'), value);
      });
    });
  });
});
